...
set indexConfPath="%windir%\GamerzOSModules\Scripts\indexConf.cmd"
if not exist %indexConfPath% (
	echo The 'indexConf.cmd' script wasn't found in GamerzOSModules.
	if "%~1"=="" pause
	exit /b 1
)
...
:: Add only the Start Menu and GamerzOSDesktop paths by default
:: The GamerzOS folder is so that if the user searches for a GamerzOS-modified feature, a script shows up in search
%indexConf% /include "%programdata%\Microsoft\Windows\Start Menu\Programs"
%indexConf% /include "%windir%\GamerzOSDesktop"

...
if ($NonInteractive -and (!$UninstallEdge -and !$InstallEdge)) {
...
# AUTOMATIC MAIN LOGIC
# Only two automatic actions: 1 = Remove Edge user data, 2 = Uninstall Edge (with AppX unless KeepAppX)
# Run in order: Remove user data first, then uninstall Edge

$edgeInstalled = EdgeInstalled

# 1. Remove Edge User Data
KillEdgeProcesses

...
$packageInstall = "$windir\GamerzOSModules\Scripts\packageInstall.ps1"
...
$package = "*Z-GamerzOS-NoTelemetry-Package*"
...
function Menu {
	...
	Write-Host "This script adds or removes GamerzOS' NoTelemetry package, which removes certain telemetry components in Windows.`n" -ForegroundColor Cyan

	Write-BulletPoint @"
Removing the package restores the telemetry components, which can aid in troubleshooting. If
   resolving the issue involves removing the package, please report this as an issue to GamerzOS.`n
"@
	Write-BulletPoint @"
GamerzOSretains policies that should disable telemetry even after the package removal. These policies
